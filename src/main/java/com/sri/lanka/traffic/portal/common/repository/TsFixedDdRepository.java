...
import com.sri.lanka.traffic.portal.common.entity.TsFixedDd;

public interface TsFixedDdRepository extends JpaRepository<TsFixedDd, String>{
	...
	@Query(value="SELECT "
			...
			+ "			COALESCE (AVG(tfo.avg_speed),0) AS \"avg_speed\" "
			+ "			FROM "
			+ "				statistics_time_date "
			+ "			LEFT JOIN srlk.ts_fixed_onhr tfo ON CAST(RIGHT(tfo.stats_yymmdt,2) AS numeric) = statistics_time_date.statistics_time "
			+ "			AND LEFT(tfo.stats_yymmdt,8) = :searchDate "
			+ "			GROUP BY statistics_time_date.statistics_time "
			+ "			ORDER BY statistics_time_date.statistics_time "
			+ ") RESULT "
			, nativeQuery = true)
	Map<String, String> getFixedAvgSpeed(@Param("searchDate") String searchDate);
	...
	Map<String, String> getFixedVolumeStats(@Param("startDate") String startDate, @Param("endDate") String endDate);
